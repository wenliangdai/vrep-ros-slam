...
void image_process(const sensor_msgs::ImageConstPtr& msg){
    ...
    for( size_t i = 0; i < circles.size(); i++ ){
        ...
        cv::circle( srcImage, center, radius, cv::Scalar(255,255,0), 3, 8, 0 );
    }
...
int main(int argc, char** argv)
{
    ...
    while(ros::ok()){
        if (abs(target_y - 320) < 80) {
            // target_y determines linear velocity
            velCommand.linear.x  = 0.8 + (target_y - 320) * (-0.028);
            // target_x determines whether to turn left or right
            velCommand.angular.z = 0 + (target_x - 256) * (-0.016);
